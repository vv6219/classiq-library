...
class MDFTDDispatchGUI:
    def __init__(self, root: tk.Tk):
        ...
        self.create_widgets()
        self.load_preset("Standard Benchmark (80 pts, 12 techs, 4 depots)")
    ...
    def create_widgets(self):
        ...
        # Progress bar & status
        self.prog_bar = ttk.Progressbar(sidebar, mode="indeterminate")
        self.prog_bar.pack(fill=tk.X, pady=(10, 2))

        self.status_label = ttk.Label(sidebar, text="Ready", font=("Helvetica", 8, "italic"), foreground="#7F8C8D")
    ...
    def _run_dispatch_worker(self):
        ...
        try:
            ...
            self.update_status(f"Generating problem (N={n}, K={k_total}, M={m})...", running=True)
            self.log(f"\n" + "=" * 65)
            ...
            # 1. Baseline
            self.update_status("Evaluating Baseline Legacy Dispatch...", running=True)
            t0 = time.perf_counter()
            baseline_plan = dispatch_field_technicians(depots, tasks, method="baseline_heuristic")
            base_time = time.perf_counter() - t0

            # 2. Quantum F-Means
            self.update_status("Executing 3-Tier Quantum Fuzzy Dispatch...", running=True)
            ...
            # 4. Classiq Quantum Swap-Test Telemetry
            self.update_status("Querying Classiq Quantum Simulation Telemetry...", running=True)
            try:
